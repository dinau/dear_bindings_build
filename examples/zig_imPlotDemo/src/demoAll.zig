                ...
                for (0..6)|j| {
                    if (st.id[j] == i) {
                        ...
                        if (ip.ImPlot_BeginDragDropSourceItem(slLabel.ptr, 0)) {
                            ...
                            // TODO
                            ip.ImPlot_ItemIcon_Vec4(.{.x = vec4.x, .y = vec4.y, .z = vec4.z, .w = vec4.w}); ig.ImGui_SameLine();
...
fn demo_AutoFittingData() !void {
    ...
    _ = ig.ImGui_CheckboxFlagsIntPtr("ImPlotAxisFlags_AutoFit##X",  &st.xflags, ip.ImPlotAxisFlags_AutoFit); ig.ImGui_SameLine();
    ...
    _ = ig.ImGui_CheckboxFlagsIntPtr("ImPlotAxisFlags_AutoFit##Y", &st.yflags, ip.ImPlotAxisFlags_AutoFit); ig.ImGui_SameLine();
...
fn demo_DragPoints() !void {
    ...
    _ = ig.ImGui_CheckboxFlagsIntPtr("NoCursors", &st.flags, ip.ImPlotDragToolFlags_NoCursors); ig.ImGui_SameLine();
    _ = ig.ImGui_CheckboxFlagsIntPtr("NoFit",     &st.flags, ip.ImPlotDragToolFlags_NoFit); ig.ImGui_SameLine();
...
fn demo_Help() !void {
    ...
    ig.ImGui_Indent();
    // TODO: Crash !
    //ig.ImGui_BulletTextV("ImDrawIdx: %d-bit", @sizeOf(ig.ImDrawIdx) * 8);
    var buf:[100]u8 = undefined;
    const sRes = try std.fmt.bufPrint(&buf, "ImDrawIdx: {d}-bit", .{@sizeOf(ig.ImDrawIdx) * 8});
    ig.ImGui_BulletText(sRes.ptr);
...
fn demo_Heatmaps() !void {
    ...
    c.srand(@intFromFloat((ig.ImGui_GetTime() * 1000000)));
    for (0..size * size) |i| {
...
fn demo_Images() !void {
    ...
    if (ip.ImPlot_BeginPlot("##image", .{ .x = -1, .y = 0 }, 0)) {
        //ip.ImPlot_PlotImage("my image",ig.ImGui_GetIO().*.ifa.*.TexID, st.bmin, st.bmax, st.uv0, st.uv1, st.tint, 0);
        // TODO
...
fn demo_FilledLinePlots() !void {
    ...
    _ = ig.ImGui_Checkbox("Lines",&st.show_lines); ig.ImGui_SameLine();
