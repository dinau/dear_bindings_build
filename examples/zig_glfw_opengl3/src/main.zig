...
pub fn main () !void {
  ...
  var counter: i32 = 0;
  // Back ground color
  const clearColor = try allocator.alloc (f32, 4);
  clearColor[0] = 0.25;
  clearColor[1] = 0.55;
  clearColor[2] = 0.9;
  clearColor[3] = 1.0;
  // Text buffer
  var sBuf: [200:0]u8 = undefined;
  for (sBuf,0..)|_,i| {
    sBuf[i] = 0;
  }
  ...
  while (ig.glfwWindowShouldClose (window) == 0) {
    ...
    if ( ig.ImGui_Begin (c.ICON_FA_THUMBS_UP ++ " ImGui: Dear Bindings", null, 0)) {
      defer ig.ImGui_End ();
      ig.ImGui_Text (c.ICON_FA_COMMENT ++ " GLFW v"); ig.ImGui_SameLine ();
      ...
      ig.ImGui_Spacing();
      _ = ig.ImGui_InputTextWithHint("InputText","Input text here", &sBuf, sBuf.len, 0);
      ig.ImGui_Text("Input result:"); ig.ImGui_SameLine(); ig.ImGui_Text(&sBuf);
      ...
      _ = ig.ImGui_ColorEdit3 ("Clear color", clearColor.ptr, 0);
